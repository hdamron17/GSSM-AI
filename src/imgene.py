...
def load_img(filename, greyscale=True):
    # greyscale else unchanged
    im = cv2.imread(filename, 0 if greyscale else -1)
    if im is None:
...
def mate(mom, dad):
    ...
    for index in np.ndindex(*shape):
        ...
        # replace baby attribute with one from mom or dad
        baby[index] = random.choice((mom[index], dad[index]))
    return baby
...
def mutate_element(im, rate, std_dev=20):
    ...
    if random.random() <= rate:
        ...
        # TODO possibly use a different random for speed
        return im + random.gauss(0, std_dev)
    else:
        return im  # no change
...
def evolve(model_im, goal_fitness_per_pixel, pop_size=20, mutation_rate=0.01, std_dev=20, show=False, elite=True, save_freq=-1, log=True):
    ...
    while best[0] > goal_fitness:
        ...
        print("Info: Gen %s Best Fitness = %s" %
...
def new_gen(model_im, pop, mutation_rate, std_dev, elite):
    ...
    new_pop_items = [mate_and_mutate(pop[i][1], pop[j][1], mutation_rate, std_dev)
                     for i, j in pairings(fitnesses, num=num)]
...
def pairings(fitnesses, num):
    ...
    while len(pairs) < num:
        # 2-tuple with 2 randomly chosen indices
        pairs.append(np.random.choice(
            indices, size=2, replace=True, p=weights))
    return pairs
...
def test_main():
    ...
    recreated = evolve(im, goal_fitness_per_pixel=1, pop_size=20, mutation_rate=0.0002,
                       std_dev=20, show=False, elite=True, save_freq=10, log=True)
    plt.ioff()
